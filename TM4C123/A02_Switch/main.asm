 ...
 ; Program Version: 1.0
 ;
 ; Program Description: This program turns on an LED when switch one
 ;                      is pressed and turns it off when switch two
 ;                      is pressed
 ;
...
loop
	; read all port F switches
	BL 	PortF_Input

	; if SW1 pressed, turn on LED
	CMP R0, #0x01
	BEQ swpressed

	; if SW2 pressed, turn off LED
	CMP R0, #0x10
	BEQ nopressed
